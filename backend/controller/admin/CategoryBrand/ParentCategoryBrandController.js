...
const createParentCategoryBrand = async (req, res) => {
  const t = await sequelize.transaction();
  try {
    const nameAlreadyExist = await ParentCategoryBrand.findOne({
    ...
    if (nameAlreadyExist) {
      ...
      const createQuery = await ParentCategoryBrand.create(
        {
          name: req.body.name,
          createdAt: new Date(),
      ...
      const updatedQuery = await ParentCategoryBrand.findOne({
...
const getParentCategoryBrand = async (req, res) => {
  try {
    // console.log("getParentMenu - ");
    // console.log("admin_id - ", req.admin.id);
    const query = await ParentCategoryBrand.findAll({
      include: [
        {
          model: ChildCategoryBrand,
          required: false,
...
const updateParentCategoryBrand = async (req, res) => {
  const t = await sequelize.transaction();
  try {
    const nameAlreadyExist = await ParentCategoryBrand.findOne({
      where: { name: req.body.name, admin_id: req.admin.id },
    ...
    if (nameAlreadyExist) {
      ...
      const [updated] = await ParentCategoryBrand.update(
        {
          name: req.body.name,
          updatedAt: new Date(),
        },
      ...
      if (updated) {
        ...
        const query = await ParentCategoryBrand.findOne({
          where: { id: req.params.id, admin_id: req.admin.id },
          transaction: t,
        });
        await t.commit();
...
const deleteParentCategoryBrand = async (req, res) => {
  const t = await sequelize.transaction();
  try {
    const query = await ParentCategoryBrand.destroy({
      where: { id: req.params.id, admin_id: req.admin.id },
      transaction: t,
    });
...
module.exports = {
  createParentCategoryBrand,
  getParentCategoryBrand,
  updateParentCategoryBrand,
  deleteParentCategoryBrand,
};
